...
function CategoriesRow({ category }) {
  ...
  const categoryAvailable = calculateCategoryAvailable(
    categoryEnriched.activity,
    categoryEnriched.budget
  )

  const countSelectedSubcategories = (selectedSubcategory) => {
    const isSubcategoryToggled = selectedSubcategories.filter(
              ...
              <Table aria-label="purchases" style={{ tableLayout: 'fixed' }}>
                <TableBody>
                  <CategoriesSubRows
                    ...
                    onCheckboxClick={countSelectedSubcategories}
                  />
                  <TableRow>
                    <TableCell
                      colSpan={7}
